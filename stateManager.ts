...
class GlobalStateManager {
    ...
    public async syncWithCloud(): Promise<void> {
        try {
            const cloudTickets = await fetchTickets(this.state.currentSport);
    ...
    public setSystemState(s: SystemState) { this.state.systemState = s; this.notify(); }
    ...
    public async addSignals(signals: MatchDashboardData[], module: ModuleType) {
        ...
        for (const data of signals) {
            ...
            const ticket: BetTicket = {
                ...
                timestamp: Date.now(), status: BetStatus.PENDING,
                isFireSignal: data.isFireSignal || data.winProbability >= 85
            };
        ...
        if (failCount > 0) {
            this.logActivity('SYSTEM', `⚠️ ${failCount} registros fallaron al subir a la nube`, 'high');
        }
