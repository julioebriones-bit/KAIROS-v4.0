...
import { 
  ...
  Search, Sparkles, Brain, History, Hash
} from 'lucide-react';
...
export const HistoryModal: React.FC<HistoryModalProps> = ({ 
  ...
  const [sortBy] = useState<'timestamp' | 'edge' | 'stake'>('timestamp');
  ...
  const filteredTickets = useMemo(() => {
    ...
    filtered.sort((a, b) => {
      const aVal = sortBy === 'timestamp' ? (a.timestamp || 0) : (a as any)[sortBy];
      const bVal = sortBy === 'timestamp' ? (b.timestamp || 0) : (b as any)[sortBy];
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-virtus-bg border border-virtus-aztecCyan/30 w-full max-w-6xl max-h-[90vh] rounded-2xl shadow-[0_0_80px_rgba(0,0,0,1)] flex flex-col relative overflow-hidden text-gray-200">
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between bg-black/40 relative z-10">
          <div>
            <h2 className="text-xl font-black text-white flex items-center gap-3 tracking-tighter uppercase tracking-[0.1em]">
              <History className="w-6 h-6 text-virtus-aztecCyan" />
              REGISTROS NEURALES <span className="text-virtus-aztecRed">V4.0</span>
            </h2>
            <p className="text-[10px] text-gray-500 font-mono mt-1 uppercase tracking-widest">
              Auditoría de Ineficiencias de Mercado - Bilateral Analysis Active
            </p>
          </div>
        ...
        {/* Filters Panel */}
        <div className="p-4 bg-slate-900/50 border-b border-white/5 grid grid-cols-1 md:grid-cols-4 gap-4 relative z-10 shadow-inner">
          <input 
            type="text" 
            placeholder="Buscar equipo o predicción..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-sm text-white outline-none focus:border-virtus-aztecCyan shadow-inner"
          />
          <select 
            value={selectedStatus} 
            onChange={(e) => setSelectedStatus(e.target.value as any)}
            className="bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-sm text-white shadow-inner"
          >
          ...
          <div className="flex gap-2 items-center px-4 bg-black/30 rounded-lg border border-white/5 shadow-inner">
          ...
          <button 
            ...
            className={`rounded-lg font-black text-[10px] uppercase tracking-widest transition-all ${showOnlyFireSignals ? 'bg-red-500 text-white shadow-[0_0_15px_rgba(239,68,68,0.5)]' : 'bg-white/5 text-gray-400 border border-white/10 shadow-inner'}`}
          >
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-slate-950 text-[10px] font-black uppercase text-gray-500 tracking-widest border-b border-white/10 z-20 shadow-[0_5px_15px_rgba(0,0,0,0.5)]">
              <tr>
                ...
                <tr key={t.id} className="hover:bg-white/5 transition-colors group">
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-1 h-8 rounded-full ${t.isFireSignal ? 'bg-red-500 shadow-[0_0_5px_red]' : 'bg-virtus-aztecCyan shadow-[0_0_5px_#00f3ff]'}`}></div>
                      <div>
                        <div className="text-sm font-bold text-white flex items-center gap-2 uppercase font-mono tracking-tighter">
                        ...
                        <div className="text-[9px] font-mono text-gray-500 flex items-center gap-2 mt-1">
                          <span className="text-virtus-aztecCyan uppercase font-black">{t.module}</span>
                          <span>•</span>
                          <span>{t.timestamp ? new Date(t.timestamp).toLocaleDateString() : 'N/A'}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="p-4">
                    <span className="text-xs font-mono font-black text-gray-300 bg-white/5 px-2 py-1 rounded border border-white/5">{t.prediction}</span>
                  </td>
        ...
        {/* Footer */}
        <div className="p-4 border-t border-white/10 bg-black/60 flex items-center justify-between text-[10px] font-mono text-gray-500 relative z-10">
          <div className="flex gap-4">
            <span className="flex items-center gap-1"><Brain className="w-3 h-3 text-virtus-aztecCyan" /> KAIROS_V4_AUDIT</span>
            <span className="flex items-center gap-1"><Hash className="w-3 h-3" /> TOTAL_OPS: {tickets.length}</span>
          </div>
          <div className="animate-pulse flex items-center gap-2 text-virtus-aztecCyan">
            <div className="w-2 h-2 rounded-full bg-virtus-aztecCyan shadow-[0_0_5px_#00f3ff]"></div>
