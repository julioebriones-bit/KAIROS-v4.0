...
import { Cpu, Zap, Activity, ShieldCheck, RefreshCw, WifiOff, Cloud, CheckCircle } from 'lucide-react';
import { SystemState, SystemHealthData } from '../types';
...
export const Header: React.FC<HeaderProps> = ({ 
  ...
  const getSystemStateColor = () => {
    switch (state) {
      ...
      case SystemState.AUTONOMOUS_CYCLE:
  ...
  const getSystemStateText = () => {
    switch (state) {
      ...
      case SystemState.AUTONOMOUS_CYCLE: return 'AUTONOMOUS_CYCLE';
    ...
    <header className="h-20 border-b border-virtus-aztecCyan/30 bg-black/90 backdrop-blur-xl flex items-center justify-between px-8 sticky top-0 z-50 overflow-hidden">
      ...
      <div className="flex items-center gap-5 cursor-pointer group" onClick={onReset}>
        ...
        <div>
          <h1 className="font-mono font-black text-xl tracking-[0.2em] text-white flex items-center gap-3 uppercase">
            KAIROS<span className="text-virtus-aztecRed text-glow-cyan">V4</span>
          </h1>
          <div className="flex items-center gap-2 mt-1">
            <div className={`w-2 h-2 rounded-full ${state === SystemState.ANALYSIS_ACTIVE || state === SystemState.AUTONOMOUS_CYCLE ? 'bg-virtus-aztecCyan shadow-[0_0_5px_#00f3ff]' : 'bg-emerald-500 shadow-[0_0_5px_#10b981]'} animate-pulse`}></div>
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 flex items-center gap-6">
        <div className="flex flex-col items-center">
          ...
          <div className={`flex items-center gap-2 px-3 py-1 rounded-full ${!isHealthy ? 'bg-amber-500/10 border border-amber-500/30' : 'bg-emerald-500/10 border border-emerald-500/30'} shadow-inner`}>
            <div className={`w-2 h-2 rounded-full ${!isHealthy ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`}></div>
            <span className={`text-[10px] font-mono font-bold ${!isHealthy ? 'text-amber-400' : 'text-emerald-400'}`}>
      ...
      <div className="flex items-center gap-6">
        ...
        {healthData?.metrics && (
          <div className="hidden lg:flex flex-col items-end">
            <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest mb-1">Pending Analysis</span>
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800/50 border border-slate-700/50 shadow-inner">
              <span className="text-[10px] font-mono font-bold text-virtus-accent">
                {healthData.metrics.pending_tickets}
              </span>
            </div>
          </div>
        )}
